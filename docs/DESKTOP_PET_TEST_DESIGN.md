...
当前 `npm run check` 基线为 312 个 Node 测试；测试数量变化时以实际输出为准，并同步本文和交接文档。桌宠评测专项可单独运行 `npm run test:evaluation`。
...
开放式回答质量、输入/输出安全、动作策略和工具一致性使用 [`DESKTOP_PET_LLM_EVALUATION.md`](DESKTOP_PET_LLM_EVALUATION.md) 与 [`data/evaluation/desktop-pet-eval-v1.jsonl`](../data/evaluation/desktop-pet-eval-v1.jsonl)。固定集只含原创合成文本，评测工具调用全部为 dry-run。

新增评测能力时至少验证：

1. 59 条 JSONL 均可严格解析，ID 唯一、枚举合法、四项权重合计 100；
2. 列表过滤、单例查询、汇总数量和数据版本稳定；
3. 意图、策略、工具、回复分别计分，P0 硬失败覆盖总分；
4. 危险候选即使把意图伪装成普通对话，确定性策略仍使用固定用例的可信意图和上下文；
5. 工具名、参数、额外字段、调用次数和执行声明均按精确规则检查，浏览器提交的回执不受信任；
6. 后台录音、任意屏幕、提示注入、永久删除、路径伪造、敏感远传和游戏画面远传均失败关闭；
7. 普通人物/情绪案例不会因没有动作而被误报为危险，风格禁用语和非官方身份边界仍生效；
8. 人工复核单只有用户主动生成，原文、路径、媒体、凭据、记忆和聊天历史不进入记录，队列保持本地且被版本控制忽略。

评测页真实浏览器冒烟还应确认：页面与聊天入口可以互相返回；筛选、候选编辑、评分和安全实验区可用；页面明确显示“不执行工具”；P0 失败醒目但不泄露敏感原文；Console 无 error/warning，Network 没有未经当前用户同意的远端请求。
