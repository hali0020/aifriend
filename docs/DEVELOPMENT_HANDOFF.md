...
- 主界面新增“评测”工作台：加载 `data/evaluation/automotive-eval-v1.jsonl` 的 62 条原创合成固定用例，可按优先级/类别/分组筛选，查看输入、车辆上下文、期望与 Rubric，粘贴候选结果评分，或让当前本地 Ollama 模型在无工具连接的 dry-run 沙盒中生成候选结构。
- 车辆安全链路把输入内容、隐私/注入、意图、确定性动作策略、工具与输出声明分别判定。行驶中开门等请求允许“意图正确”，但策略必须阻断，工具不得执行；阻断后声称成功同样是 P0。人工复核单只保留白名单字段、哈希和脱敏短片段，用户主动生成后写入被忽略的 `data/evaluation-review-queue.local.json`。
...
- 当前自动化基线为 244 个 Node 测试（以最新 `npm run check` 输出为准），另有桌面启动策略、实际服务/模型冒烟、浏览器页面检查和 Python 语料/素材验证；主界面、桌宠与回复策略的可执行矩阵见 `docs/DESKTOP_PET_TEST_DESIGN.md`，汽车测试与评测安全规范见 `docs/AUTOMOTIVE_LLM_TEST_PLAN.md`、`docs/LLM_EVALUATION_AND_SAFETY.md`。
...
9. 已加入汽车座舱大模型评测基线：62 条原创 JSONL 用例、意图/策略/工具/回复四维确定性评分、车辆动作策略、输入多轴风险、输出执行声明校验、页面/API、本地 dry-run，以及有界脱敏人工复核队列。P0 错误不能被平均分抵消；页面和接口没有连接真实车辆或通用工具。
...
8. **真实车辆与量产安全验证**：当前汽车用例、策略和页面只运行在无副作用评测沙盒，不能替代车型能力矩阵、可信车辆状态源、硬件在环、故障注入、功能安全/网络安全流程和地区法规认证。接入任何实际车控前需要独立安全控制器与人工安全评审；人工复核不得作为行驶中的即时远程放行机制。
...
4. 修改前先运行相关专项测试；修改后至少运行 `npm run check`。需要真实 Agent/模型链时，先启动 `npm start`，再运行 `npm test`。
   - 只修改汽车评测、策略或页面时，可先运行 `npm run test:evaluation`；策略变化仍须重新运行全部 P0 固定集。
5. 新增桌宠静态 PNG 时：
...
- 车辆意图解析不能直接授权副作用。门锁/后备箱/驾驶席/驾驶区视频等动作必须经过独立确定性策略；状态缺失、过期或冲突时失败关闭，`block`/`manual_review` 时工具调用数为 0，未核实执行结果时不得声称成功。
- `data/evaluation-review-queue.local.json` 只能保存受白名单和二次脱敏处理的复核记录，不保存原始对话、精确位置、VIN、联系信息或凭据，不得自动上传；人工复核用于事后治理，不用于即时放行危险动作。
