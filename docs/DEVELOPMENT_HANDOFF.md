...
- 当前自动化基线为 137 个 Node 测试（以最新 `npm run check` 输出为准），另有桌面启动策略、实际服务/模型冒烟、浏览器页面检查和 Python 语料/素材验证。
...
1. **真实 GUI 长时间运行**：在用户自己的 Windows PowerShell 运行 `npm run desktop:diagnose`，覆盖真实 GPU、多显示器/DPI、休眠唤醒和窗口频繁开关；如崩溃，保留本机诊断目录和 Windows 事件中的故障模块。自动化环境不能代替此结论。
