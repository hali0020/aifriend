...
import { redactReviewText, reviewTextContainsSensitiveData } from "./desktop-pet-action-policy.js";

const DEFAULT_MAX_RECORDS = 200;
const DEFAULT_MAX_FILE_BYTES = 2 * 1024 * 1024;
const ALLOWED_STATUS = new Set(["pending", "in_review", "resolved", "accepted_risk", "duplicate", "invalid"]);
const SENSITIVE_KEY = /(?:raw|password|passwd|secret|token|api[_-]?key|private[_-]?key|authorization|cookie|location|latitude|longitude|coordinate|address|email|phone|path)/iu;
...
export function sanitizeReviewRecord(record) {
  ...
  const sanitized = {};
  for (const key of ROOT_KEYS) if (Object.hasOwn(record, key)) sanitized[key] = sanitizeValue(record[key], key);
  ...
  sanitized.decision = ["allow", "confirm", "block", "manual_review"].includes(record.decision) ? record.decision : "manual_review";
  const submittedExecution = isPlainObject(record.toolExecution) ? record.toolExecution : {};
  sanitized.toolExecution = {
    attempted: submittedExecution.attempted === true,
    verified: false,
  };
  sanitized.privacy = { redacted: true, rawTextStored: false, localOnly: true };
...
export function createEvaluationReviewStore({
  ...
  async function readRecords() {
    ...
    let parsed;
    try { parsed = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(source)); } catch { throw new Error("Human-review queue is not valid UTF-8 JSON"); }
  ...
  async function count() { return (await readRecords()).length; }
...
export const EVALUATION_REVIEW_LIMITS = Object.freeze({ maxRecords: DEFAULT_MAX_RECORDS, maxFileBytes: DEFAULT_MAX_FILE_BYTES });
