...
let image = null, audio = null, recorder = null, chunks = [], chatController = null, activeRequestOrigin = "";
let desktopPet = null;
...
let gameStream=null,gameTimer=null,gamePaused=false,gameLastAnalysis=0,gameLastSignature=null;
...
function compactSafety(verdict){
  ...
  return{phase:verdict.phase==="output"?"output":"input",action:verdict.action,severity:safetySeverities.has(verdict.severity)?verdict.severity:"medium",categories,source:verdict.source==="local+openai"?"local+openai":"local",remoteUsed:verdict.remoteUsed===true};
}
...
function refreshStatus(){fetch("/api/status").then(r=>r.json()).then(s=>{
  $("#status").textContent=s.connected?`${s.local?"本地":"云端"} · ${s.model}`:"演示模式 · 可直接体验";
  const safety=$("#safetyStatus"),privacy=$("#privacyState");
  if(s.safety?.remoteEnabled){safety.textContent=s.safety.remoteReady?"本地文本输入/输出检查 · 可选 OpenAI 远程复核已启用":"本地文本输入/输出检查 · 可选远程复核配置不完整";safety.dataset.mode=s.safety.remoteReady?"remote":"error"}
  else{safety.textContent="本地文本输入/输出检查 · 可选远程复核未启用";safety.dataset.mode="local"}
  privacy.textContent=s.processing==="cloud"?"☁️ 云端模式 · 对话会发送给所选云服务":"🔒 本地优先 · 不会自动回退云端";
...
async function loadModels(){
  ...
  $("#asrState").textContent=speech.ready?"已安装 · 录音时按需加载":"运行库已安装 · 语音模型待下载";
...
async function loadGameStatus(){
  ...
  gameModelReady=supported&&status.ready;$("#gameLocalState").textContent=!supported?"当前浏览器不支持屏幕共享":status.ready?`本地视觉模型已就绪 · ${status.model} · 截图不落盘`:`${status.model} 尚未就绪，请先启动 Ollama 或安装视觉模型`;updateGameUi();return{supported,status,ready:gameModelReady};
}
...
async function startGameSession(){
  ...
  gameStarting=true;let candidate=null,finalMessage="";updateGameUi("正在确认本地视觉模型");
  try{
    const check=await loadGameStatus();if(!check.ready){finalMessage="本地视觉模型不可用";return}if(gameStream)await stopGameSession();saveGamePrefs();const token=++gameSessionId;$("#gameLocalState").textContent="等待你选择游戏窗口…";
    candidate=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:{ideal:2,max:5}},audio:false});if(token!==gameSessionId)throw gameCaptureAbort();gameStream=candidate;gamePaused=false;gamePreview.srcObject=candidate;await gamePreview.play();if(token!==gameSessionId||gameStream!==candidate)throw gameCaptureAbort();
    const stream=candidate,track=stream.getVideoTracks()[0];if(!track)throw new Error("没有取得可用的视频轨道");track.addEventListener("ended",()=>{if(gameStream===stream)stopGameSession()},{once:true});candidate=null;finalMessage=`共享中 · ${track.label||"已选择画面源"} · ${gamePrefs.automatic?`每 ${gamePrefs.interval} 秒观察一帧`:"自动观察关闭"}`;$("#gameLocalState").textContent="画面源已共享；模型只会收到你触发的离散截图";scheduleGameObservation();
  }catch(error){candidate?.getTracks().forEach(track=>track.stop());if(gameStream===candidate)gameStream=null;gamePreview.pause();if(!gameStream)gamePreview.srcObject=null;if(!["NotAllowedError","AbortError"].includes(error.name))alert(`无法共享画面：${error.message}`);$("#gameLocalState").textContent=error.name==="NotAllowedError"?"你取消了屏幕共享":error.name==="AbortError"?"共享已停止":"屏幕共享启动失败";finalMessage="没有开始共享"}finally{gameStarting=false;updateGameUi(finalMessage)}
}
...
async function captureGameFrame(token=gameSessionId){
  const stream=gameStream;if(!stream||gamePreview.readyState<2)throw new Error("共享画面还没有准备好");
  const ensureCurrent=()=>{if(token!==gameSessionId||gameStream!==stream)throw gameCaptureAbort()},width=gamePreview.videoWidth,height=gamePreview.videoHeight,scale=Math.min(1,1280/Math.max(width,height)),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(width*scale));canvas.height=Math.max(1,Math.round(height*scale));canvas.getContext("2d",{alpha:false}).drawImage(gamePreview,0,0,canvas.width,canvas.height);const signature=gameSignature(canvas);
  const encode=quality=>new Promise(resolve=>canvas.toBlob(resolve,"image/jpeg",quality));let blob=await encode(.76);ensureCurrent();if(blob?.size>2*1024*1024){blob=await encode(.55);ensureCurrent()}if(!blob||blob.size>2*1024*1024)throw new Error("当前画面压缩后仍超过 2MB");const dataUrl=await readData(blob);ensureCurrent();return{name:`game-frame-${Date.now()}.jpg`,dataUrl,signature};
}
async function analyzeGameFrame(automatic=false){
  ...
  try{const frame=await captureGameFrame(token),difference=gameDifference(gameLastSignature,frame.signature);if(automatic&&difference<3.5){gameLastSignature=frame.signature;gameLastAnalysis=new Date();finalMessage="画面变化很小，已跳过模型分析";return true}gameLastSignature=frame.signature;const text=automatic?"低频观察当前游戏画面；只有画面本身存在明显、紧急或高价值信息时才提醒。":"观察当前游戏画面，先告诉我现在最值得注意的一件事，再给下一步建议。",sent=await sendTurn({text,image:{name:frame.name,dataUrl:frame.dataUrl},game:gamePayload(automatic),origin:automatic?"game-auto":"game-manual"});if(token!==gameSessionId)return false;if(sent){gameLastAnalysis=new Date();finalMessage=automatic?"自动观察完成":"当前画面分析完成"}else finalMessage=automatic?"自动观察失败；稍后会按设定间隔重试":"画面分析失败，请检查本地视觉模型";return sent}catch(error){if(error.name!=="AbortError")finalMessage=`分析失败 · ${error.message}`;return false}finally{if(gameCaptureBusy===token)gameCaptureBusy=0;if(token===gameSessionId)updateGameUi(finalMessage)}
}
function stopGameSession(release=true){
  if(gameStopping&&!gameStream)return gameStopPromise;++gameSessionId;clearGameTimer();if(activeRequestOrigin.startsWith("game"))chatController?.abort();gameStream?.getTracks().forEach(track=>track.stop());gameStream=null;gamePaused=false;gamePreview.pause();gamePreview.srcObject=null;gameLastAnalysis=0;gameLastSignature=null;$("#gameLocalState").textContent="共享已停止";updateGameUi("共享已停止");if(!release)return Promise.resolve();gameStopping=true;updateGameUi("正在释放本地视觉模型");gameStopPromise=fetch("/api/game/stop",{method:"POST"}).catch(()=>null).finally(()=>{gameStopping=false;updateGameUi("共享已停止")});return gameStopPromise;
}
...
$("#gameAuto").addEventListener("change",()=>{saveGamePrefs();$("#gameInterval").disabled=!gamePrefs.automatic;gamePaused=false;updateGameUi();scheduleGameObservation()});applyGamePrefs();updateGameUi();window.addEventListener("beforeunload",()=>{gameStream?.getTracks().forEach(track=>track.stop());disposePetVisibility?.();desktopPet?.dispose()});
...
async function sendTurn(turn={}){
  const text=String(turn.text||"").trim(),sentImage=turn.image||null,sentAudio=turn.audio||null,game=turn.game||null,origin=turn.origin||"chat",gameFrame=origin.startsWith("game"),automatic=origin==="game-auto";
  ...
  const controller=new AbortController();chatController=controller;activeRequestOrigin=origin;desktopPet?.begin(gameFrame?"正在观察你共享的游戏画面":"先核对事实和假设");$("#send").textContent="■";$("#send").title="停止生成";
  ...
  const registerInputSafety=verdict=>{
    ...
    if(!automatic&&!gameFrame&&!inputSaved&&["allow","warn"].includes(verdict.action)){inputSaved=saveHistory("user",shownUserText,verdict)}
  ...
  try{
    const payload=gameFrame?{text,image:sentImage,game}:{text,image:sentImage,audio:sentAudio,game,history},options={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),signal:controller.signal};
    ...
    if(r.status===409&&!game){
      ...
      if(!inputStopped()&&!gameFrame)saveHistory("assistant",full,data.safety?.output);desktopPet?.complete(full,data.demo?"演示回复":"备用回复");return true;
    }
...
$("#clear").onclick=()=>{if(!confirm("清空当前对话并重新开始吗？"))return;if(gameStream||activeRequestOrigin.startsWith("game"))stopGameSession();chatController?.abort();chatController=null;stopSpeaking();history=[];image=null;audio=null;input.value="";input.style.height="auto";attachment.classList.add("hidden");document.querySelector(".waiting")?.remove();clearInterval(stateTimer);storage.remove(historyKey);desktopPet?.complete("好了，现在重新说明你的问题。尽量把条件说完整。","记录已清空");messages.innerHTML='<div class="welcome"><div class="orb avatar"><img src="/christina-avatar.webp" alt="克里斯提娜头像"></div><h2>记录已清空</h2><p>好了，现在重新说明你的问题。<br>尽量把条件说完整。</p></div>'};
